...
{                          
    ...
    //while(GPIO_ReadInputData(GPIOG)&0x80);
...
void img_disp (const unsigned char *img)
{
  ...
  for(j=0;j<32;j++)
  {
    for(i=0;i<8;i++)
    {
     LCD_WriteCommand(ay+j);
      LCD_WriteCommand(ax1+i);
    LCD_WriteData(img[j*16+i*2]);
     LCD_WriteData(img[j*16+i*2+1]);
    }
  }
 LCD_WriteCommand(ay-32);
  LCD_WriteCommand(ax2);
  for(j=32;j<64;j++)
  {
    for(i=0;i<8;i++)
    {
     LCD_WriteCommand(ay+j-32);
     LCD_WriteCommand(ax2+i);
     LCD_WriteData(img[j*16+i*2]);
     LCD_WriteData(img[j*16+i*2+1]);
    }
  }
...
/*******************************************************************************
* Function Name  : void LCD_Display_Fresh(void)
* Description    : LCD刷新显示函数;
* Description    : LCD_flag为界面号: 0=>LCD_Start; 1=>LCD_Main; 2=>LCD_Set; 3=>LCD_State; 4=>坐标点图;
* Input          : None
* Output         : None
* Return         : LCD_Display_Fresh();								   
*******************************************************************************/
