...
/*******************************************************************************
* Function Name  : void TIM1_PWM_Init(void)
* Description    : TIM1配置PWM输出
* Description    : PE12(T1_CH3N,outA)  PE9(T1_CH1,outB)  PE8(T1_CH1N,outC)  PE11(T1_CH2,outD) 
* Input          : None
* Output         : None
* Return         : TIM1_PWM_Init();
*******************************************************************************/
...
/*******************************************************************************
* Function Name  : TIM2_12864_Init
* Description    : 定时器TIM2初始化,液晶显示时间隔一秒闪烁;
* Input          : None
* Output         : None
* Return         : None										   
*******************************************************************************/
...
/*******************************************************************************
* Function Name  : void TIM3_Cap_Init(void)
* Description    : TIM3通道3/4输入捕获配置: TIM3_CH3, TIM3_CH4;
* Input          : None
* Output         : None
* Return         : TIM3_Cap_Init();
*******************************************************************************/
...
{ 
	if (TIM_GetITStatus(TIM3, TIM_IT_CC3) != RESET)//捕获3发生捕获事件
		...
		//输出X立个Flag
			OUTX_Flag=1;
			//如果X右移，则OUTX_Num++,用于记录距离
			if(MOT_DirX==1)				OUTX_Num++;
			
			//如果X左移，则OUTX_Num++,用于记录距离
			else if(MOT_DirX==2)
			{
	...
	if (TIM_GetITStatus(TIM3, TIM_IT_CC4) != RESET)//捕获4发生捕获事件
		{
			//输出Y立个Flag
			OUTY_Flag=1;
			//如果X上移，则OUTY_Num++,用于记录向上距离
			if(MOT_DirY==1)				OUTY_Num++;
			
			//如果X下移，则OUTY_Num++,用于记录向上距离
			else if(MOT_DirY==2)
			{
			...
			Run_X = (OUTX_Num*4)/330;		//减速比差值	计算此次为距离
			Run_Y = (OUTY_Num*8)/330;		//减速比差值	计算此次为距离
...
/*******************************************************************************
* Function Name  : void TIM4_PWM_Init(void)
* Description    : TIM4配置PWM输出;TIM4_CH3,TIM4_CH4  Z轴电机,舵机;
* Input          : None
* Output         : None
* Return         : TIM4_PWM_Init();
*******************************************************************************/
