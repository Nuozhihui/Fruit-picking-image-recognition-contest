...
{
	BSP_RCC_Configuration();			//RCC时钟初始化
    BSP_GPIO_Configuration(); 	////蜂鸣器,摇杆电位器(包括按键）,矩阵键盘 HL,电机使能,串口 初始化
	BSP_EXTI_Configuration();			//中断初始化
}
...
{
	//蜂鸣器,摇杆电位器(包括按键）,矩阵键盘 HL,电机使能,串口
						 ...
						 case 9:
							 ...
							 if(RxTmp==0xfe)
							 {
								 ...
								 if(RDatNum>0) 
								 {
									 ...
									 //被追踪目标坐标X,Y>起点坐标X,Y	&&	被追踪目标坐标<终点坐标X,Y
